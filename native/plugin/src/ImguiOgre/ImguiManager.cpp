...
namespace AVImgui{
    ...
    void ImguiManager::destroySingleton() {
        delete msSingleton;
        msSingleton = 0;
    }

    ImguiManager::ImguiManager() : mPSOCache(0),
                                   mSceneMgr(0),
                                   mDisplayTarget(0),
                                   mPass(0),
                                   mFontTex(0),
    ...
    void ImguiManager::init(Ogre::SceneManager* sceneManager, Ogre::TextureGpu* displayTarget) {
    ...
    bool ImguiManager::wouldBeFirstUpdateOfFrame() const {
        if (!mFrameActive) return true;
        unsigned long ogreFrame = Ogre::Root::getSingletonPtr()->getNextFrameNumber();
        return ogreFrame != mFrameStartedOgreFrame;
    }

    void ImguiManager::ensureFrameStarted() {
        unsigned long ogreFrame = Ogre::Root::getSingletonPtr()->getNextFrameNumber();
        if (mFrameActive && ogreFrame == mFrameStartedOgreFrame) {
            //A frame is already in flight for this engine frame.
            return;
        }

        //If a previous frame was started but never rendered (for instance the
        //workspace was disabled) it has to be ended before a new one can begin.
        if (mFrameActive) {
            ImGui::EndFrame();
            mFrameActive = false;
        }
        ...
        if (mHasFrameTime) {
            deltaTime = std::chrono::duration<float>(now - mLastFrameTime).count();
            if (deltaTime <= 0.0f) deltaTime = 1.0f / 600.0f;
    ...
    void ImguiManager::render(Ogre::RenderPassDescriptor* renderPassDesc, Ogre::TextureGpu* renderTarget) {
    #ifdef __APPLE__
        @autoreleasepool {
    #endif
        ...
        renderSystem->beginRenderPassDescriptor(renderPassDesc, renderTarget, 0, &viewportSize, &scissors, 1, false, false);
        renderSystem->executeRenderPassDescriptorDelayedActions();

        if (mLastPsoDescriptor != renderPassDesc) {
            // Lots of stuff can be done only once for the sake of efficiency.
            mPSOCache->clearState();

            mPSOCache->setRenderTarget(renderPassDesc);
            ...
            mLastPsoDescriptor = renderPassDesc;
        }

        ImDrawData* drawData = ImGui::GetDrawData();
        unsigned int numberDraws = 0;

        int vpWidth = renderTarget->getWidth();
        int vpHeight = renderTarget->getHeight();
        ...
        for (int n = 0; n < drawData->CmdListsCount; n++) {
            ...
            for (int i = 0; i < drawList->CmdBuffer.Size; i++) {
                ...
                mRenderables[numberDraws]->updateVertexData(vtxBuf, &idxBuf[startIdx], drawList->VtxBuffer.Size, drawCmd->ElemCount);
                ...
                mRenderables[numberDraws]->getRenderOperation(renderOp, false);
                ...
                renderSystem->bindGpuProgramParameters(Ogre::GPT_VERTEX_PROGRAM, mPass->getVertexProgramParameters(), Ogre::GPV_ALL);

                //imgui 1.92: ImTextureID is an integer type and ImDrawCmd::TextureId
                //was replaced with GetTexID().
                Ogre::TextureGpu* texGpu = reinterpret_cast<Ogre::TextureGpu*>(static_cast<uintptr_t>(drawCmd->GetTexID()));
                renderSystem->_setTexture(0, texGpu, false);
                renderSystem->_setHlmsSamplerblock(0, mSamplerblock);
    ...
    void ImguiManager::createMaterial() {
        ...
        Ogre::HighLevelGpuProgramPtr vertexShaderVK = mgr.getByName("imgui/VP/vulkan");
        Ogre::HighLevelGpuProgramPtr pixelShaderVK = mgr.getByName("imgui/FP/vulkan");
    ...
    void ImguiManager::createFontTexture() {
        ...
        //imgui 1.92: ImTextureID is an integer type.
        io.Fonts->SetTexID((ImTextureID)(uintptr_t)mFontTex);
